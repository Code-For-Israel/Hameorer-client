    ...
    const pickDocument = async () => {
        ...
        if (result.type === "success") {
            if (ALLOWED_TYPES.includes(result.mimeType) === false)
                ...
                const userGroupBody = {
                    group_name: result.file.name, users: excelParsedToJsonData,
                };
    ...
    function ShowParsedData() {
        return (<>{
            parsedData && parsedData.users && parsedData.users.length >= 1 ? (<>
                <Text style={{textDecorationLine: "underline"}}>Items from excel that will be uploaded</Text>
