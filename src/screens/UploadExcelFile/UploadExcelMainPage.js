...
export function UploadExcelMainPage({setParsedData, parsedData}) {
    ...
    useEffect(() => {
        if (uploadFileConst && uploadFileConst.error) console.log("error", uploadFileConst);
    }, [uploadFileConst]);
    ...
    const showFileDetails = () => {
        if (fileUpload !== null) return <Text>File to upload: {fileUpload.name}</Text>; else return <Text></Text>;
    };
    ...
    async function uploadFile() {
        ...
        const userGroupBody = {
            group_name: fileUpload.name, users: excelParsedToJsonData,
        };
    ...
    return (<>
            {<View style={{alignItems: "center", justifyContent: "center"}}>
                ...
                {showFileDetails()}
                <>
                    {parsedData ? (<Text>File parsed - watch console log to see if uploaded</Text>) : ("")}
